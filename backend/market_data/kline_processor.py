        ...
        for code in Crypto_codes:
            ...
            for week_end, row in weekly.iterrows():
                ...
                if existing is None:
                    ...
                    week_start_data = (
                        df[df.index >= week_start].iloc[0]
                        if len(df[df.index >= week_start]) > 0
                        else None
                    )
        ...
        for code in Crypto_codes:
            ...
            for month_end, row in monthly.iterrows():
                ...
                if existing is None:
                    ...
                    month_start_data = (
                        df[df.index >= month_start].iloc[0]
                        if len(df[df.index >= month_start]) > 0
                        else None
                    )
