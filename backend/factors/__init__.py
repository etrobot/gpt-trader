    ...
    for mod_name in _iter_factor_modules():
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Failed to import factor module {mod_name}: {e}"
            )
    return factors


def compute_all_factors(
    history: Dict[str, pd.DataFrame], top_symbols: Optional[pd.DataFrame] = None
) -> pd.DataFrame:
    """Compute all registered factor DataFrames and outer-join them by 'symbol'."""
    dfs: List[pd.DataFrame] = []
    for factor in list_factors():
        try:
            df = factor.compute(history, top_symbols)
...
def compute_selected_factors(
    ...
    top_symbols: Optional[pd.DataFrame] = None,
    ...
    if selected_factor_ids is None:
        return compute_all_factors(history, top_symbols)
